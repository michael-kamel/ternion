...
function constructBuild({buildSpec, errorHandler, tools, opts})
{
    ...
    Object.keys(buildSpec.events).map(evtName =>
    {
        if(buildSpec.events[evtName].preValidators)
            handler.registerPreValidator(evtName, ...buildSpec.events[evtName].preValidators)
        if(buildSpec.events[evtName].middlewares)
            handler.registerMiddleware(evtName, ...buildSpec.events[evtName].middlewares)
        if(buildSpec.events[evtName].postValidators)
            handler.registerPostValidator(evtName, ...buildSpec.events[evtName].postValidators)
    ...
    builds.forEach(bld =>
    {
        Object.keys(bld.getHandler()._events).forEach(evtName =>
        {
            ...
            if(bld.getHandler()._events[evtName][typeName])
                handler._events[evtName][typeName] = handler._events[evtName][typeName].concat(bld.getHandler()._events[evtName][typeName])
        })
    })
}
